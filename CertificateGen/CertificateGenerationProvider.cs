...
namespace CertificateGen
{
    public class CertificateGenerationProvider
    {
        public void GenerateRootCertificate(CertificateConfiguration settings)
        {
            ...
            RsaKeyGenerationParameters prms =
                new RsaKeyGenerationParameters(new Org.BouncyCastle.Math.BigInteger("10001", 16), secureRandom, 1024, 4);
            keyGen.Init(prms);
            ...
            try
            {
                using (FileStream fs = new FileStream(p12FileName, FileMode.Create))
                {
                    ...
                    p12.Save(fs, settings.Password.ToCharArray(), secureRandom);
                    fs.Close();
                }
            }
